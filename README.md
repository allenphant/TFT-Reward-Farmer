...
* 請確保遊戲介面語言與截圖庫一致（或自行替換 `assets` 內的圖片）。

### 2. 安裝依賴
請確保已安裝 Python 3，並在終端機執行以下指令以安裝所需套件：
...
### 3. 設定圖片路徑
打開 src/main.py，找到 PATH 變數，將其修改為您電腦上 assets 資料夾的實際絕對路徑：

```Python
# 例如：
PATH = r'C:\Users\YourName\Documents\TFT-Reward-Farmer\assets'
```
